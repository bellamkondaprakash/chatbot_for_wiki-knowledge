# Chatbot_Tensorflow 
Chatbot with data imported from DBpedia's wikipedia dataset and Reddit data using Tensorflow v1.5 and Python v3.6.1
...
- **Install [TensorFlow](http://www.tensorflow.org/)** for Python 3. To run a pretrained model, the CPU-only installation should suffice for using any pretrained model. If you want to train your own models, you'll need the GPU installation of TensorFlow (and a powerful CUDA-compatible GPU).
...
#### For pretrined model run the script at /model directory named as pretrained_models.sh
```
bash  /model/pretrained_models.sh
```
